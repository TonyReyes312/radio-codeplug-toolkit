...
This isn't a standalone program you double-click. It's a toolkit meant to be handed to an AI assistant (Claude, ChatGPT, or similar) that can execute Python. You don't run these scripts yourself; you paste Codeplug_Prompt.md into a chat with the AI, attach your CSV, your original codeplug file, & generic_channel_parser.py plus the relevant writers/*.py module, and the AI runs the code & hands back the finished file.

The prompt file exists because an AI starting from zero, with just "convert this CSV to a Baofeng codeplug," will burn through several failed attempts rediscovering the file format's quirks (see below). Codeplug_Prompt.md hands over that knowledge up front, plus the already-working code, so the AI can produce a correct file on the first pass instead of re-learning the format by trial and error.

If you'd rather run the Python yourself without an AI in the loop, the code works fine that way too. The "Usage" section below is the same either way.
